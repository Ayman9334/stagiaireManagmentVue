...
const creeEtab = () => {
    ...
    axiosClient.post('/etablissement', req)
        .then(() => {
            axiosClient.get('/etablissement').then(({ data }) => { etablissements.value = data });
...
const modifierEtab = () => {
    ...
    axiosClient.put(`/etablissement/${methodId.value}`, req)
        .then(() => {
            axiosClient.get('/etablissement').then(({ data }) => { etablissements.value = data });
            const button = document.getElementById('close-creation-modal');
            button.click();
...
const suprimerEtab = (id) => {
    confirm.require({
        ...
        accept: () => {
            axiosClient.delete(`/etablissement/${id}`)
                .then(() => {
                    axiosClient.get('/etablissement').then(({ data }) => { etablissements.value = data });
                })
        }
...
const desctiverEtab = (id) => {
    axiosClient.get(`/etablissement/desactiver/${id}`)
        .then(() => {
            axiosClient.get('/etablissement').then(({ data }) => { etablissements.value = data });
        })
}
...
<template>
    <section class="etablissement p-3">
        <!-- creation form -->
        <div class="modal fade" id="etab-form" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="w-100 my-3 d-flex bg-light p-2 border rounded justify-content-between align-items-center">
            ...
            <div>
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#etab-form"
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr v-for="(etablissement, key) in etablissements" :key="key">
                        <td>{{ etablissement.libelle }}</td>
                        <td>{{ etablissement.adresse }}</td>
                        <td>
                            <ConfirmPopup></ConfirmPopup>
                            <button class="btn btn-danger me-1" @click="suprimerEtab(etablissement.id)">
                                <i class="far fa-trash"></i>
                            </button>
                            <button class="btn btn-info me-1 text-white" @click="desctiverEtab(etablissement.id)">
                                <i class="fas fa-eye-slash"></i>
                            </button>
                            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#etab-form"
                                @click="reqFacory('modifier', etablissement)">
